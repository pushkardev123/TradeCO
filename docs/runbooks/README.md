...
| [Order Command Processing and Redis Streams](order-command-processing-and-redis-streams.md) | Initial structure; Redis Streams sections pending | Backend and execution owners | Diagnose command processing and future Redis Stream stuck-message handling. |
...
- Current checked-in order command transport uses Redis Pub/Sub channels. The target architecture is Redis Streams, so stream-specific recovery steps remain pending until that migration exists in code.
- Current checked-in credential storage is encrypted fields on `User`. The target architecture is a separate `ExchangeCredential` model, so model-specific rotation steps remain pending until that migration exists in code.
